...
js_analysis() {
  ...
  if require_tool subjs && [ -s "$WORKDIR/03_live_hosts/live.txt" ]; then
    ...
    cat "$WORKDIR/03_live_hosts/live.txt" | subjs > "$OUT/subjs_output.txt" 2>/dev/null
  ...
  if command -v mantra &>/dev/null && [ -s "$JS_URLS" ]; then
    ...
    cat "$JS_URLS" | mantra > "$OUT/mantra_output.txt" 2>/dev/null
  ...
  cp "$JS_URLS" "$OUT/js_urls.txt" 2>/dev/null
  ...
  while IFS= read -r js_url; do
    ...
    if [ -n "$content" ]; then
      ...
      echo "$content" | grep -Ei \
        ...
        >> "$OUT/extracted_secrets.txt" 2>/dev/null
  ...
  # Deduplicate outputs
  sort -u "$OUT/extracted_endpoints.txt" -o "$OUT/extracted_endpoints.txt" 2>/dev/null
  sort -u "$OUT/extracted_secrets.txt" -o "$OUT/extracted_secrets.txt" 2>/dev/null
  ...
  log info "Endpoints extracted: $(wc -l < "$OUT/extracted_endpoints.txt" 2>/dev/null | tr -d ' ')"
  log info "Potential secrets: $(wc -l < "$OUT/extracted_secrets.txt" 2>/dev/null | tr -d ' ')"
}
