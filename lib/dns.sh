...
dns_resolution() {
  ...
  if require_tool dnsx; then
    ...
    dnsx -l "$IN" -o "$OUT/resolved.txt" -silent -retry 3 -threads 200 2>/dev/null
    check_output "$OUT/resolved.txt" "dnsx"
  fi

  # ASN mapping
  if require_tool asnmap; then
    log info "Running ASN mapping..."
    asnmap -d "$TARGET" -silent > "$OUT/asn_mapping.txt" 2>/dev/null
    ...
    if [ -s "$OUT/asn_mapping.txt" ]; then
      ...
      while IFS= read -r cidr; do
        echo "$cidr" | dnsx -silent -resp-only -ptr >> "$OUT/ptr_records.txt" 2>/dev/null
      done < "$OUT/asn_mapping.txt"
      sort -u "$OUT/ptr_records.txt" -o "$OUT/ptr_records.txt" 2>/dev/null
    fi
  fi
  ...
  if require_tool hakrevdns && [ -s "$OUT/resolved.txt" ]; then
    ...
    cut -d' ' -f1 "$OUT/resolved.txt" 2>/dev/null | \
      hakrevdns -d "$TARGET" -t 200 >> "$OUT/ptr_records.txt" 2>/dev/null
    sort -u "$OUT/ptr_records.txt" -o "$OUT/ptr_records.txt" 2>/dev/null
  fi
